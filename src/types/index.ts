...
export interface NutritionMatch {
  ...
  input: string;
  /**
   * Нормализованное имя, которое выделил парсер («курица бедро»). Нужно как
   * ключ алиаса при разрешении (calculate матчит алиасы именно по parsed-имени,
   * не по сырой строке). Null для пропущенных парсером строк.
   */
...
export interface NutritionData {
  ...
  /**
   * Цифры заданы автором вручную (а не посчитаны по составу через USDA).
   * Для составных/нетиповых блюд. В UI помечаем «указано вручную» — чтобы не
   * выдавать ручные числа за USDA-точность ±5%.
   */
  manual?: boolean;
