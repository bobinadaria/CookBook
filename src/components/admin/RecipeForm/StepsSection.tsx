...
function StepRow({ step, index, isFirst, isLast, onChange, onRemove, onMoveUp, onMoveDown }: StepRowProps) {
  ...
  return (
    <div className="bg-crust/50 border border-rule rounded-none p-4 flex gap-4">
      {/* Order + reorder controls */}
      <div className="flex flex-col items-center gap-1 shrink-0 pt-1">
        <span className="font-display text-2xl text-muted leading-none w-7 text-center">
          {index + 1}
        </span>
        <button type="button" onClick={onMoveUp} disabled={isFirst}
          className="p-1 rounded text-muted hover:text-burg disabled:opacity-20 transition-colors">
      ...
      <div className="flex-1 flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3">
          {preview ? (
            <div className="relative w-16 h-16 rounded-none overflow-hidden shrink-0">
            ...
            <button type="button" onClick={() => photoInputRef.current?.click()}
              className="flex items-center gap-2 text-xs text-soft hover:text-ochre-dk transition-colors border border-dashed border-rule hover:border-ochre-dk rounded-none px-3 py-2">
      ...
      <button type="button" onClick={onRemove}
        className="p-1.5 self-start rounded-none text-muted hover:text-red-400 hover:bg-red-50 transition-colors">
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-4">
        <label className="block text-xs text-soft uppercase tracking-wider">
          Шаги приготовления
        </label>
        <span className="text-xs text-muted">{steps.length} шагов</span>
      </div>
      ...
      <button type="button" onClick={onAdd}
        className="flex items-center gap-2 text-sm text-soft hover:text-ochre-dk transition-colors border border-dashed border-rule hover:border-ochre-dk rounded-none px-4 py-3 w-full justify-center">
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
        </svg>
        Добавить шаг
