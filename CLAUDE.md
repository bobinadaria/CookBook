...
10. **Нерешённые UX-находки** (из раунда 2, не критичные, но ждут): формулировка «обложки
    отдельными пакетами» на `/pricing` непонятна (2/2); фраза FAQ «прежде чем платить, прочитай»
    пугает после оплаты (2/2); метафора «глава/номер» на главной неясна (2/3). Лог —
    `ux-testing/UX_Findings_Summary.md`.
11. **Копирайтинг главной + `/pricing`** — отдельная сессия, бриф зафиксирован в
    `RECIPE_IMPORT_AND_PREMIUM_TEASERS_PLAN.md` §5. Главная ценность: личная книга как единый
