...
- **Модалка «имя ИЛИ ссылка»** — `UserQuickCreateModal` с переключателем режимов; в режиме «Ссылка»
  импорт прямо в модалке (sessionStorage → форма), `PremiumLock` для Free; фикс backdrop-бага (F45).
- **PremiumLock** — компонент `src/components/ui/PremiumLock.tsx`; обёрнуты 3 секции `UserRecipeForm`
  (ссылка, обложка, КБЖУ) + режим «Ссылка» в модалке. Free видит фичи, но в locked-состоянии.
- **Приватность книги** — `emptyPrivacyNote` в `MyBookEmptyState`; note про приватность в кабинете.
- **ScrollToHash** — `src/components/pricing/ScrollToHash.tsx`; плавный скролл к `#covers` / `#plans`.
...
- Копирайтинг главной + `/pricing` — отдельная сессия (бриф в `RECIPE_IMPORT_AND_PREMIUM_TEASERS_PLAN.md` §5).
...
✅ **Закрыто 2026-06-26 (лаунч-сессия):**
10. ~~/privacy + /terms~~ — Server Components, язык из next-intl, без дублирующих кнопок.
11. ~~Stripe Checkout consent~~ — consent_collection + custom_text с ссылками ToS/PP.
12. ~~Favicon~~ — ST-монограмма (favicon.ico + icon.png + apple-icon.png).
13. ~~/pricing копирайт~~ — тёмная тема для всех, без счётчиков мест, честный леде Premium.
14. ~~Фикс stripe_customer_id~~ — Test→NULL в Supabase, Live-клиенты создаются автоматически.

**Активный бэклог (следующее):**
- **Instagram анонс 2026-07-03** — пост + сторис (видео); следующая сессия.
- **Контент** — единицы измерения, сложные ингредиенты (лемонграсс и т.п.) — Дарья сама.
- **Premium-фичи** по одной («скоро» на `/pricing`): PDF-экспорт → AI-генерация рецепта → шеринг по ссылке.
...
- **`docs/smm-content-guide.md`** — МАСТЕР по SMM: тон, форматы постов, структура карусели, темы, чеклист, частота. ЧИТАТЬ перед любым ответом про соцсети, раскрутку или контент-стратегию.
- **`docs/social-media-positioning-brief.md`** — быстрый бриф позиционирования (три кита, сильные фразы, платформы). Открывать перед написанием конкретного поста.
- **`docs/instagram-launch-2026-07-03.md`** — готовый контент для анонса (карусель, пост, сторис, reels).
